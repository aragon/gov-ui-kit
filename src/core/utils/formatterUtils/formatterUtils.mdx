...
The `DURATION` format returns the difference between a specified date and now, expressed in the largest unit with a value
of 1 or more. That unit is rounded to the nearest whole number, and smaller units are not shown. For example: 1hr 53min
12sec → '2 hours'
